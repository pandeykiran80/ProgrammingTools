...
x = Comb_matU[n,0]
y = Comb_matU[n,1]
xi = np.linspace(min(Comb_matU[n,0]), max(Comb_matU[n,0]))
yi = np.linspace(min(Comb_matU[n,1]), max(Comb_matU[n,1]))
zi = scipy.interpolate.griddata((x,y),Comb_matU[n,1],(xi,yi),method="linear")

mpl.rcParams['xtick.direction'] = 'out'
mpl.rcParams['ytick.direction'] = 'out'
plt.imshow(zi, vmin=Comb_matU[n,1].min(), vmax=Comb_matU[n,1].max(), origin="lower", 
